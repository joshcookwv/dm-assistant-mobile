...
**Known issue to fix (found by tester):** the Worker currently consumes a client's rate-limit quota *before* checking whether the Worker itself is healthy/correctly configured — so a broken deploy would burn everyone's daily limit on server-fault 500s for nothing. Fix: check config/health first, only decrement the KV counter once the request is confirmed attemptable.
